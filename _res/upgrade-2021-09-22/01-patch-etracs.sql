...
INSERT INTO `sys_var` (`name`, `value`, `description`, `datatype`, `category`) 
VALUES ('liquidate_remittance_as_of_date', '1', 'Liquidate Remittance as of the specified date', 'checkbox', 'TC');



drop table if exists ztmp_fix_ugm; 
drop table if exists ztmp_fix_ugm_0; 

create table ztmp_fix_ugm
select t0.*, 
  (
    select objid from sys_usergroup_member
    where user_objid = t0.user_objid  
      and usergroup_objid = t0.usergroup_objid 
      and ifnull(org_objid,'') = ifnull(t0.org_objid,'') 
    limit 1 
  ) as ugmid  
from ( 
...
;
create table ztmp_fix_ugm_0
select ugm.* 
from ztmp_fix_ugm z, sys_usergroup_member ugm 
where ugm.user_objid = z.user_objid 
  and ugm.usergroup_objid = z.usergroup_objid 
  and ifnull(ugm.org_objid, '') = ifnull(z.org_objid, '')
  and ugm.objid <> z.ugmid 
; 
delete from sys_usergroup_member where objid in (
  select objid from ztmp_fix_ugm_0 
